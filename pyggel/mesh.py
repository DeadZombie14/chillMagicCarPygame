...
import os
from pyggel import image, view, data, misc, math3d
        ...
        if values[0] in ("o", "g"):
            ...
            for line in open(os.path.join(path, values[1]), "r"):
                ...
                if values[0] == 'newmtl':
                    ...
                    cur_mtl.set_color(list(map(float, values[1:])))
        elif values[0] == 'f':
            face = []
            texcoords = []
            norms = []
...
class Animation(BaseSceneObject):
    ...
    def __init__(self, meshx, skeleton, commands):
    ...
    def copy(self):
        ...
        new = Animation(self.mesh1, self.skeleton, com)
        new.pos = self.pos
        new.rotation = self.rotation
        new.scale = self.scale
