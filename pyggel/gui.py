...
from pyggel import view, font, event, misc
import time
from pyggel import image as _image
import os
...
class Theme(object):
    ...
    def load(self, filename):
        ...
        if misc.test_safe(filename)[0]:
            ...
            # exec ( "g={%s}"%open(filename, "rU").read() )
            # g = { str( open(filename, "rU").read()) }
            # for widget in g:
            #     for val in g[widget]:
            #         self.theme[widget][val] = g[widget][val]
            
            # Step 2
            with open(filename, 'wb') as new_theme:
                # Step 3
                pickle.dump(self.theme, new_theme)
